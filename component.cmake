...
# 输入commontent的名字或别名，获取对应的commontent目标。
function(__component_get_target var name_or_alias)
    # Look at previously resolved names or aliasesss
    idf_build_get_property(component_names_resolved __COMPONENT_NAMES_RESOLVED)
    list(FIND component_names_resolved ${name_or_alias} result) # 查找名称或别名，返回对应索引，存于resault，无则resault=-1
    if(NOT result EQUAL -1)  # 有这个名
        ...
        set(${var} ${target} PARENT_SCOPE) # 定义一个commontent target变量
        return()
    endif()

    idf_build_get_property(component_targets __COMPONENT_TARGETS)

    # Assume first that the paramters is an alias. 假设是个别名
...
function(__component_add component_dir prefix)
    ...
    # 函数功能：
    #   创建comment_target和component_target library目标
    idf_build_get_property(component_targets __COMPONENT_TARGETS) # 载入__idf_build_target的__COMPONENT_TARGETS属性
    get_filename_component(abs_dir ${component_dir} ABSOLUTE)
    get_filename_component(base_dir ${abs_dir} NAME) # commponent文件夹名。

    if(NOT EXISTS "${abs_dir}/CMakeLists.txt") # 对应的commonent须有CMakeLists.txt
        message(FATAL_ERROR "Directory '${component_dir}' does not contain a component.")
    endif()

    set(component_name ${base_dir})
    # The component target has three underscores三条下横线 as a prefix. The corresponding component library
    # only has two.
    # 1 创建相应commontent变量只在函数内部用。
    set(component_target ___${prefix}_${component_name}) # 定义对应commontent变量。
    ...
    # 2 创建commontent_target库目标。用于产生commontent.a库，__COMPONENT_TARGETS添加一个属性值。
    #   这里的component_target存在三种不同含义：
    #     1）$(component_target)是一个变量
    #     2）component_target是字符串，可作为__COMPONENT_TARGETS属性值之一，__COMPONENT_TARGETS是多值的。
    #     3）作为TARGET域的一个目标。如：add_library(${component_target} STATIC IMPORTED)
    if(NOT component_target IN_LIST component_targets) # component_target不是__idf_build_target的__COMPONENT_TARGETS属性值。
        if(NOT TARGET ${component_target}) # 如果component_target 不在TARGET(目标)域中。
            add_library(${component_target} STATIC IMPORTED) # 创建一个component_target目标，注意使用这个目标要象使用变量一样${component_target}
        endif()
        idf_build_set_property(__COMPONENT_TARGETS ${component_target} APPEND) # 把commontent_target附加进__idf_build_target的__COMPONENT_TARGETS属性中
    endif()
    ...
    # Set the basic properties of the component 为目标commonent_target添加以下属性
