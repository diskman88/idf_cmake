...
# call.
# 配置一些内部build属性，在idf_build_process调用前，这些属性可修改。
...
__target_init() # 定义在target.cmake中，定义target相关变量。
...
# 暂时重载project()，project()重载后，重载前的函数仍然可以通过调用__project()使用。
function(project)
endfunction()
...
macro(project project_name)
    ...
    # 初始化项目，准备COMPONENTS相关参数给idf_build_process()调用。
    __project_init(components test_components)

    # 最终执行toochain-esp32.cmake 设置编译变量
    ...
    # The actual call to project() 实际调用cmake project()执行的动作
    __project(${project_name} C CXX ASM) # 设置project名字和编译语言，名字存在PROJECT_NAME中，
    ...
    # 由于组件可以导入第三方库，原始的project()须在组件加入build前保存。
    function(project)
    ...
    #idf_build_process()定义在build.cmake
