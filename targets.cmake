...
# 涉及三个变量：env_idf_target IDF_TARGET(cache中) ENV{IDF_TARGET}（环境变量）需要同步它们
#
macro(__target_init)
    # Input is IDF_TARGET environement variable
    # 1、设置变量env_idf_target，把ENV{IDF_TARGET}环境变量赋值给它，ENV{IDF_TARGET}在idf.py中配置
    set(env_idf_target $ENV{IDF_TARGET}) 

    # 2、env_idf_target为空？
    if(NOT env_idf_target) # 1) 为空，idf.py没配置。
        # IDF_TARGET not set in environment, see if it is set in cache
        if(IDF_TARGET)
            set(env_idf_target ${IDF_TARGET})
        else()
            set(env_idf_target esp32) # 如果cache也没有，配置成缺省值esp32
